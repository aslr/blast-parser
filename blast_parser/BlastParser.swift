...
extension BlastParser {
    ...
    struct MergeMinimap: ParsableCommand {
        static let configuration = CommandConfiguration(
            ...
            usage: "blast_parser merge-minimap --hits <minimaphits> --stats <minimapstats> --blasthits <blasthits> --reads <fasta-file> [--output <output>]",
            aliases: ["mrgm"] )
        
        @OptionGroup var options: Options
        
        @Option(name: [.short, .customLong("hits")],
                help: "Path to the minimap2 output table containing the following columns: QueryID, Reference_ID, Alignment_Score, Alignment_Length and Taxonomy or the directory containing files whose name has the suffix '_representative_classified.tsv' (e.g., Sample_200_representative_classified.tsv) generated by the 'parse-minimap' subcommand. Multiple file or directory paths, separated by spaces, can be given to merge multiple tables.")
        var minimapHits:String
        
        @Option(name: [.short, .customLong("stats")],
                help: "Path to the minimap2 output stats table containing the following columns: Taxonomy, Hit_Count, Maximum_Score, Minimum_Score or the directory containing files whose name has the suffix '_stats.tsv' (e.g., Sample_200_stats.tsv) generated by the 'parse-minimap' subcommand.")
        var minimapStats:String
        
        @Option(name: [.short, .customLong("reads")],
                help: "Path to the fasta file containing the reads that were selected by the 'parse-minimap' subcommand.")
        var reads:String
