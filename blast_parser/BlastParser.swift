...
extension BlastParser {
	...
	struct MergeQiime: ParsableCommand {
		...
		@Option(name: [.short, .customLong("asvs")],
				help: "Path(s) to table file(s) generated by Qiime2 containing ASVs and their respective read counts and taxonomic classification separated by space(s). It will use the name of the ASV file directory to prefix the respective taxonomy assignment and confidence column headers.")
		var asvs:String
		...
		mutating func run() throws {
			guard let parser = BlastQiimeOutputParser(path: blasthits, asvs: asvs)
			else {
